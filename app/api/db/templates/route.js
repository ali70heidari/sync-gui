...
export async function GET() {
  try {
    ...
    const templates = (await db.template.findMany({ orderBy: [{ parentTemplateId: 'asc' }, { id: 'asc' }] })).map(asTemplate);
...
export async function POST(request) {
  try {
    ...
    const { id, name, parent_template_id, relative_path, relative_remote_path, variable_keys, hidden } = body;
