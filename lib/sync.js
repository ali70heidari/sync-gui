...
export async function runSync({ direction = 'up', dryRun = false, noDelete = false, itemTargets = {} }) {
  ...
  for (const item of items) {
    ...
    if (direction === 'up') {
      for (const ti of targetIndices) {
        ...
        const missingRemoteIds = remoteIds.filter(id => !config.remotes.some(r => r.id === id));
        if (missingRemoteIds.length) {
          const targetName = target.name || `target ${ti}`;
          const missingStr = missingRemoteIds.map(id => `"${id}"`).join(', ');
          throw new Error(`Item "${item.name}" / ${targetName} references unknown remote ${missingStr}. Select an existing remote for this target.`);
        }
        ...
        for (const remote of remotes) {
          ...
          if (result.code === 0 && !dryRun && target.postSyncCommand && target.postSyncCommand.trim()) {
            postUploadMappings.push({ item, target, remote, label });
          }
        }
      }
    } else {
  ...
  if (finalCode === 0 && postUploadMappings.length) {
    for (const mapping of postUploadMappings) {
      chunks.push(`[${mapping.item.name} ? ${mapping.label}] after successful upload`);
      const pResult = await runPostUploadCommand({ target: mapping.target, remote: mapping.remote });
      if (pResult.output) chunks.push(pResult.output);
      if (pResult.code !== 0 && finalCode === 0) finalCode = pResult.code;
      chunks.push(`[${mapping.item.name} ? ${mapping.label}] after-upload exit ${pResult.code}`);
    }
  }
...
async function runPostUploadCommand({ target, remote }) {
  const command = (target.postSyncCommand || '').trim();
  if (!command) return { code: 0, output: '' };
  if (remote.kind !== 'ssh') {
    return runBash(command, '');
  }
  const { sshInvocation, authPass } = sshCommandInvocation(remote);
  const sshCmd = `${sshInvocation} ${shq(`${remote.username}@${remote.host}`)} ${shq(command)}`;
  return runBashProcess(sshCmd, authPass);
}

function sshCommandInvocation(remote) {
  const port = remote.port || 22;
  const authMethod = remote.authMethod || 'password';
  const password = remote.password || '';
  const privateKey = remote.privateKeyPath || '';
  const passphrase = remote.keyPassphrase || '';
  const isKeyAuth = authMethod === 'key' || (Boolean(privateKey) && !password);

  let sshPrefix = 'ssh';
  let authPass = '';

  if (isKeyAuth) {
    const keyOpt = privateKey ? ` -i ${shq(toShellPath(privateKey))}` : '';
    if (passphrase) {
      sshPrefix = `sshpass -e ssh${keyOpt}`;
      authPass = passphrase;
    } else {
      sshPrefix = `ssh${keyOpt}`;
      authPass = '';
    }
  } else {
  ...
  const ssh = `${sshPrefix} -p ${shq(String(port))} -o StrictHostKeyChecking=accept-new${sshKnownHostsOption()}`;
  return { sshInvocation: ssh, authPass };
}
...
async function listRemoteMatches(remote, pattern) {
  ...
  const { sshInvocation, authPass } = sshCommandInvocation(remote);
  const ssh = `${sshInvocation} ${shq(`${remote.username}@${remote.host}`)} ${shq(findCommand)}`;
  const result = await runBashProcess(ssh, authPass);
  if (result.code === 0) {
...
async function syncSshExactDown({ item, remote, remotePath, localPath, noDelete }) {
  const { sshInvocation, authPass } = sshCommandInvocation(remote);
  const remoteSpec = `${remote.username}@${remote.host}:${remotePath}`;
  const ssh = sshInvocation;
  const flags = ['-azs', '--human-readable', '--itemize-changes', '--no-o', '--no-g'];
  ...
  return runBash(command, authPass);
}
...
async function copyMapping(type, src, dst, noDelete, sourceIgnores = [], destIgnores = sourceIgnores) {
  try {
    ...
    await mirrorDir(src, dst, isIgnored, isDestIgnored);
...
async function mirrorDir(src, dst, isIgnored, isDestIgnored = isIgnored, root = src, dstRoot = dst) {
  ...
  for (const e of srcEntries) {
    ...
    if (e.isDirectory()) await mirrorDir(s, d, isIgnored, isDestIgnored, root, dstRoot);
