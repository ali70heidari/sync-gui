...
async function syncLocal({ item, target, remote, direction, dryRun, noDelete }) {
  ...
  for (const { src, dst } of pairs) {
    const result = await copyMapping(item.type, src, dst, noDelete, item.type === 'folder' ? item.source : null);
...
async function copyMapping(type, src, dst, noDelete, ignoreRoot) {
  try {
    ...
    const isIgnored = await readSyncIgnore(ignoreRoot);
    if (noDelete) {
...
async function mirrorDir(src, dst, isIgnored, root = src) {
  ...
  for (const e of srcEntries) {
    ...
    if (e.isDirectory()) await mirrorDir(s, d, isIgnored, root);
  ...
  for (const e of await fsp.readdir(dst, { withFileTypes: true })) {
    if (isIgnored(path.relative(root, path.join(src, e.name)))) continue;
