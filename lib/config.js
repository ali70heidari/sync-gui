...
export function getConfigPath() {
  return process.env.SYNC_CONFIG || process.env.SYNC_GUI_CONFIG || (process.cwd() + '/sync-config.json');
}
...
export async function readConfig() {
  try {
    ...
    return { remotes: [], projects: [], categories: [], items: [] };
  }
}
...
export async function writeConfig(config, options = {}) {
  ...
  const out = { remotes: config.remotes, projects: config.projects, categories: config.categories, items: config.items };
  await fs.writeFile(getConfigPath(), JSON.stringify(out, null, 2), 'utf8');
}
