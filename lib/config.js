...
export async function readConfig() {
  const { readDbConfig } = await import('./db-config.js');
...
export async function writeConfig(config) {
  const { writeDbConfig } = await import('./db-config.js');
...
function flattenCategoryMappings(rows, project, remote, category, categoryPath, inheritedVariables = {}) {
  const categoryVariables = { ...inheritedVariables, ...(category.variables || {}) };
  for (const mapping of category.mappings) {
    rows.push({
      ...
      category,
      categoryPath,
      mapping: { ...mapping, variables: { ...categoryVariables, ...(mapping.variables || {}) } }
    });
  }

  for (const child of category.categories) {
    flattenCategoryMappings(rows, project, remote, child, [...categoryPath, child.id], categoryVariables);
  }
}

function flattenStreamCategoryMappings(rows, project, remotes, stream, category, categoryPath, inheritedVariables = {}) {
  const categoryVariables = { ...inheritedVariables, ...(category.variables || {}) };
  for (const mapping of category.mappings) {
  ...
  for (const child of category.categories) {
    flattenStreamCategoryMappings(rows, project, remotes, stream, child, [...categoryPath, child.id], categoryVariables);
  }
}
