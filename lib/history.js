...
export function clearHistory() {
  ...
  store.jobs = store.jobs.filter(job => job.status === 'running');
  return before - store.jobs.length;
}

export function startSyncJob(request) {
  let itemTargets = request.itemTargets || {};
  if (!Object.keys(itemTargets).length && request.itemIds?.length) {
    itemTargets = {};
    for (const id of request.itemIds) itemTargets[id] = [];
  }
  const job = {
    ...
    itemTargets,
    itemIds: Object.keys(itemTargets),
    status: 'running',
    exitCode: null,
    output: '',
  ...
  runSync({ direction: job.direction, dryRun: job.dryRun, noDelete: job.noDelete, itemTargets })
