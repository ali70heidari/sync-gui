...
export function applyImport(existing, data, resolutions) {
  ...
  for (const r of data.remotes || []) {
    const importedId = r.id;
    const res = remoteRes[r.name];
    if (!res) {
      const copy = { ...r, id: nextId('r-', existing.remotes) };
      existing.remotes.push(copy);
      remapRemoteId[importedId] = copy.id;
      continue;
    }
    ...
    if (res.action === 'replace') {
      ...
      if (idx !== -1) {
        existing.remotes[idx] = { ...existing.remotes[idx], ...copy, id: existing.remotes[idx].id };
        remapRemoteId[importedId] = existing.remotes[idx].id;
        continue;
      }
    }
  ...
  for (const p of data.projects || []) {
    ...
    if (res.action === 'replace') {
      ...
      if (idx !== -1) {
        existing.projects[idx] = { ...existing.projects[idx], ...copy, id: existing.projects[idx].id };
  ...
  for (const category of data.categories || []) {
    const copy = {
      ...category,
      id: nextId('c-', existing.categories),
      projectId: remapProjectId[category.projectId] || category.projectId
    };
    existing.categories.push(copy);
    importedCategories.push(copy);
    remapCategoryId[category.id] = copy.id;
  }
  for (const category of importedCategories) {
    if (remapCategoryId[category.parentId]) category.parentId = remapCategoryId[category.parentId];
  }
  ...
  for (const item of data.items || []) {
    ...
    const copy = {
      ...
      categoryId: remapCategoryId[item.categoryId] || item.categoryId || ''
    };
